...
3人家族（夫・妻・義母）向けの車シェア予約LINE Bot。LINEで「明日の午前」のように送ると予約でき、Botのキャラクター「ポピィ」が累計予約数に応じて50段階に「進化」していくゲーミフィケーション付き（[functions/src/lib/rankTable.js](functions/src/lib/rankTable.js)）。予約はLINEの1:1トークのテキストコマンドと、リッチメニューから開くLIFF（LINE製のミニアプリ）のタップ操作の両方から行える。
